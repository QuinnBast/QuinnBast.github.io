  ...
  - name: CalianAlerts
    rules:
      ...
      - alert: CalculationBelowZero
        ...
        expr: sum(calian_sampleClientApp_calculation_value) < 0
...
![Alert](./images/ConfiguredAlert.png)

Here we can click on the expression to view the data returned by the expression. However, some of you may see this:

![No Data Alert](./images/NoDataAlert.png)

Why? This is because of our query. Our query is `sum(calian_sampleClientApp_calculation_value) < 0` which is searching for a metric of values less than zero.
...
![Active Alert](./images/ActiveAlert.png)
...
Navigate to the [Prometheus config file](../config/prometheus.yml) and add the following configuration:
