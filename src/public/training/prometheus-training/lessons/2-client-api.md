...
![Application Metrics](./images/ExportedMetrics.png)
...
![Successful Target](./images/PrometheusSuccessfulTarget.png)
...
In the project, there are only three classes:

- `CalculationException` - is a custom exception that gets thrown when calculations cause problems.
- `UnitOfWorkNoPrometheusImpl` - is an example of a class in any application that does some random unit of work.
- `PrometheusTrainingMain` - is the main application

We want to instrument our `UnitOfWorkNoPrometheusImpl` to quantify the work it is doing.
Let's open this class.

Once open, we want to register a few Counter metrics.
This is extremely easy to do, setup a new `Counter` by adding a `static` variable to the class:
...
![Custom Metric](./images/OurMetricValue.png)

There it is! Our counter is in the list of metrics and is reporting a value of `2.0`!

Your turn!

Try adding another counter on your own:

Similarly to `Foo`, the `Bar` method calls out to a function called `DoCalculation` a number of times.
Instrument a counter to count how many times we are doing these calculations!
...
Let's open `UnitOfWorkNoPrometheusImpl` and find the `DoCalculation` method.

In this method, we can see a few things are being set:
- The returned value
- The selected satellite
- If the value should be positive or negative

We want to report the *value* of everything returned from this dunction as a metric, but we also want to be able to filter out and query based on the satellite and if it was positive or not.
To do this, we need to add a label to isolate data based on it's direction and satellite.
...
We use the `labelNames` field to indicate that this gauge should be able to be filtered by the `positive` and `satellite` fields.
In relational database terms, adding `labels`, lets you perform a `WHERE` clause on these two fields.

Now, in the `DoCalculation`, we can record the returned value along with the fields relating to the value we collect:
